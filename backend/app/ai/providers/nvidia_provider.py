...
_shared_nvidia_client: Optional[httpx.AsyncClient] = None


def get_shared_nvidia_client() -> httpx.AsyncClient:
    """Returns a persistent httpx AsyncClient singleton with connection pooling for NVIDIA."""
    global _shared_nvidia_client
    if _shared_nvidia_client is None or _shared_nvidia_client.is_closed:
        _shared_nvidia_client = httpx.AsyncClient(
...
async def close_nvidia_client() -> None:
    """Closes the shared httpx client during application shutdown."""
    global _shared_nvidia_client
    if _shared_nvidia_client is not None and not _shared_nvidia_client.is_closed:
        await _shared_nvidia_client.aclose()
        _shared_nvidia_client = None


class NvidiaAnalyzerProvider:
    """
    ...
    async def ping(self, timeout: float = 5.0) -> Dict[str, Any]:
        ...
        if not api_key or api_key.strip() in ("", "your_server_side_nvidia_api_key_here"):
            return {
                "name": self.name,
                "ok": False,
                "latency_ms": None,
        ...
        model_name = settings.NVIDIA_MODEL or "meta/llama-3.3-70b-instruct"
        ...
        try:
            ...
            return {
                ...
                "ok": False,
                "latency_ms": latency_ms,
                "error": f"NVIDIA API responded with status {res.status_code}",
            }
            ...
            return {
                ...
                "ok": False,
                "latency_ms": latency_ms,
                "error": f"NVIDIA ping timed out after {timeout}s",
            }
            ...
            return {
                ...
                "ok": False,
                "latency_ms": latency_ms,
                "error": f"NVIDIA ping failed: {type(e).__name__}",
            }
    ...
    async def generate_json(
        ...
        if not api_key or api_key.strip() in ("", "your_server_side_nvidia_api_key_here"):
            raise HTTPException(
                ...
                detail="NVIDIA_API_KEY is not configured on the backend server.",
            )
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail="NVIDIA AI analysis service timed out after 60 seconds. Please try again.",
            )
        except Exception as e:
            err_str = str(e).lower()
            ...
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail="NVIDIA AI analysis service encountered an error processing the request.",
            )
        ...
        if response_status == 401:
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail="Invalid NVIDIA API key configured on the backend server.",
            )
        if response_status >= 500:
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail=f"NVIDIA AI service returned server error (status {response_status}).",
            )
        if response_status != 200:
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail=f"NVIDIA AI service returned unexpected error (status {response_status}).",
            )
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail="NVIDIA AI service returned invalid response format.",
            )
    ...
    async def analyze(
        ...
        # Handle HTTP error statuses with standard mapping
        if response_status == 429:
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail="AI provider rate limit reached. Please wait a moment before analyzing again.",
        ...
        # 5. Deterministic Remediation Engine with Claim-Preservation Validation
