...
export function RoadmapMilestoneCard({
  ...
  const isReadOnly = roadmapLifecycle === "ARCHIVED" || roadmapLifecycle === "COMPLETED";
  ...
  const getReconciliationBadge = () => {
    if (!milestone.reconciliation) return null;
    switch (milestone.reconciliation.status) {
      case "GROUNDED_BY_PROMOTED_PROJECT":
        return (
          <Badge variant="outline" className="bg-emerald-500/10 text-emerald-400 border-emerald-500/30 text-[11px] font-medium gap-1">
            <CheckCircle2 className="h-3 w-3" />
            <span>Grounded by Promoted Project</span>
          </Badge>
        );
      case "GROUNDED_BY_WORKSPACE":
        return (
          <Badge variant="outline" className="bg-blue-500/10 text-blue-400 border-blue-500/30 text-[11px] font-medium gap-1">
            <CheckCircle2 className="h-3 w-3" />
            <span>Grounded by Workspace Evidence</span>
          </Badge>
        );
      case "RELATED_UNVERIFIED":
        return (
          <Badge variant="outline" className="bg-amber-500/10 text-amber-400 border-amber-500/30 text-[11px] font-medium gap-1">
            <ShieldAlert className="h-3 w-3" />
            <span>Related — Verification Required</span>
          </Badge>
        );
      case "NOT_GROUNDED":
        return (
          <Badge variant="outline" className="bg-slate-800 text-slate-400 border-slate-700 text-[11px]">
            Not Yet Grounded
          </Badge>
        );
    }
  ...
  return (
    ...
    >
      ...
      <CardContent className="p-4 pt-1 space-y-4">
        ...
        {/* Promoted Workspace Project Linkage */}
        {milestone.promotedProjectId && (
          <div className="flex items-center justify-between rounded-btn border border-emerald-500/30 bg-emerald-500/5 p-3 text-caption">
            <div className="flex items-center gap-2 text-emerald-400">
              <Link2 className="h-4 w-4 shrink-0" />
              <span>
                <strong>Linked Workspace Project:</strong> {milestone.promotedProjectId}
              </span>
            </div>
            <Link href="/workspace/projects">
              <Button size="sm" variant="outline" className="h-7 text-[11px] gap-1 text-emerald-400 border-emerald-500/40">
